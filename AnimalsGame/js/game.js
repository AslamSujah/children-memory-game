// ================================
// ANIMAL MEMORY GAME - MAIN GAME LOGIC
// ================================
// This is the core game engine for the animal memory matching game
...
// Sea & Sky Animals (Hard Level - Part 1)
const SEA_SKY_ANIMALS = [
    '🐧', '🐦', '🦅', '🦉', '🦜', '🦚',
    '🐬', '🐳', '🐋', '🦈', '🐠', '🐟',
    '🐙', '🦑', '🦞', '🦀', '🐡', '🦭'
];

// Insects & Small Animals (Hard Level - Part 2)
const SMALL_ANIMALS = [
    '🐝', '🐛', '🦋', '🐌', '🐞', '🦗',
    '🕷️', '🦂', '🦟', '🐢', '🦎', '🐍'
];
...
// Animal categories for selection
const ANIMAL_SETS = {
    easy: FARM_ANIMALS,
    medium: WILD_ANIMALS,
    hard: ALL_ANIMALS
...
// ================================
// ANIMAL NAMES FOR FEEDBACK
// ================================
const ANIMAL_NAMES = {
    '🐶': 'Dog', '🐱': 'Cat', '🐭': 'Mouse', '🐹': 'Hamster',
    '🐰': 'Rabbit', '🐮': 'Cow', '🐷': 'Pig', '🐴': 'Horse',
    '🐔': 'Chicken', '🐣': 'Chick', '🦆': 'Duck', '🐑': 'Sheep',
    '🦁': 'Lion', '🐯': 'Tiger', '🐻': 'Bear', '🐼': 'Panda',
    '🐨': 'Koala', '🦊': 'Fox', '🦝': 'Raccoon', '🐺': 'Wolf',
    '🦒': 'Giraffe', '🐘': 'Elephant', '🦏': 'Rhino', '🦛': 'Hippo',
    '🐵': 'Monkey', '🦍': 'Gorilla', '🦓': 'Zebra', '🦘': 'Kangaroo',
    '🐧': 'Penguin', '🐦': 'Bird', '🦅': 'Eagle', '🦉': 'Owl',
    '🐬': 'Dolphin', '🐳': 'Whale', '🐋': 'Blue Whale', '🦈': 'Shark',
    '🐝': 'Bee', '🦋': 'Butterfly', '🐞': 'Ladybug', '🐢': 'Turtle',
    '🦎': 'Lizard', '🐍': 'Snake'
};
...
function startGame(level) {
    console.log('🐾 Starting Animal Memory Game - Level:', level);
...
/**
 * Create the game board with animal cards
...
function createBoard(level) {
    ...
    // Select animals based on level
    const animalSet = ANIMAL_SETS[level];
    const selectedAnimals = shuffleArray(animalSet).slice(0, config.pairs);
    
    // Create pairs (each animal appears twice)
    const cardValues = [...selectedAnimals, ...selectedAnimals];
    ...
    console.log('✅ Animal board created with', shuffledCards.length, 'cards');
    console.log('🐾 Theme:', config.theme);
}

/**
 * Create a single card DOM element
 * @param {string} animal - The animal emoji to display
 * @param {number} id - Unique card ID
 * @returns {HTMLElement} Card element
 */
function createCardElement(animal, id) {
    ...
    cardBack.textContent = '🐾'; // Paw print for back
...
function handleMatch(cardId1, cardId2) {
    const animal = gameState.cards[cardId1].value;
    const animalName = ANIMAL_NAMES[animal] || 'Animal';
    
    console.log('✅ Match found!', animalName);
...
function completeGame() {
    console.log('🎉 Animal Game Complete!');
...
function createConfetti() {
    ...
    // Use animal emojis for confetti!
    const animalConfetti = ['🐾', '🦴', '💚', '🌟', '⭐', '✨'];
    ...
    for (let i = 0; i < 50; i++) {
        ...
        // Mix colors and animal emojis
        if (Math.random() > 0.5) {
            confetti.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)];
            confetti.textContent = '';
        } else {
            confetti.textContent = animalConfetti[Math.floor(Math.random() * animalConfetti.length)];
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🐾 Animal Memory Game Loaded');
